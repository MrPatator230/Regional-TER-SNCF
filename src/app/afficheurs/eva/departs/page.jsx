...
export default function AfficheurEVAArrivees(){
    ...
    // Enrichir les departures avec les perturbations quotidiennes (si présentes)
    useEffect(()=>{
        if(!data || !Array.isArray(data.departures) || !data.departures.length) return;
        ...
        const enriched = data.departures.map((d)=>{
            try{
                ...
                const perturbation = perturbations.find((p)=>{
                    ...
                    if(pSid !== sId) return false;
                    if(pDate){
                        if(pDate !== visibleDateStr) return false;
                    }
                    return true;
                });
        ...
        try{
            if(JSON.stringify(enriched) !== JSON.stringify(data.departures)){
                setData(prev => ({ ...prev, departures: enriched }));
            }
        }catch(e){ /* ignore stringify errors */ }
    }, [perturbations, data?.departures, dateParam]);
    ...
    const departures = data?.departures || data?.arrivals || [];
    const pageSize = 4;
    ...
    // Fonction de filtrage par date (utilise les times de départ)
    function filterForDate(refDate){
        ...
        return departures.filter(d=>{
            ...
            const {stop} = getStopForGare(d);
            // Filtrer les départs dans la journée
            if(!matchesServiceDays(d, refDate)) return false;
            // heure efficace: expected or planned (prioriser departure fields)
            const plannedRaw = (stop && (stop.departure_time_planned || stop.planned_departure_time || stop.planned_time)) || d.planned_departure_time || d.planned_time || d.scheduled_departure_time || d.scheduled_time || null;
            const expectedRaw = (stop && (stop.departure_time || stop.departure_time_real || stop.departure_time_estimated)) || d.expected_departure_time || d.departure_time || d.expected_time || null;
            ...
            // trier par heure effective asc (prioriser champs de départ)
            const sa = getStopForGare(a).stop; const sb = getStopForGare(b).stop;
            const ea = effectiveDateOn((sa && (sa.departure_time || sa.departure_time_real || sa.planned_departure_time || sa.planned_time)) || a.departure_time || a.expected_departure_time || a.planned_departure_time || a.planned_time, refDate);
            const eb = effectiveDateOn((sb && (sb.departure_time || sb.departure_time_real || sb.planned_departure_time || sb.planned_time)) || b.departure_time || b.expected_departure_time || b.planned_departure_time || b.planned_time, refDate);
    ...
    let filtered = filteredToday.filter(d=>{
        const {stop} = getStopForGare(d);
        const plannedRaw = stop?.departure_time_planned || stop?.planned_departure_time || stop?.planned_time || d.planned_departure_time || d.planned_time || d.scheduled_departure_time || d.scheduled_time || null;
        const expectedRaw = stop?.departure_time || stop?.departure_time_real || d.expected_departure_time || d.departure_time || d.expected_time || null;
    ...
    const visibleDepartures = (filtered || []).slice(0, pageSize);
        ...
        <div className="eva-root">
            <header className="eva-topbar">
                <div className="eva-left">
                    ...
                    <div className="eva-title">
                        <div className="main">DÉPARTS</div>
                        <div className="sub">/ DEPARTURES FROM / SALIDAS HASTA</div>
                    </div>
                </div>
            ...
            <main className="eva-board">
                ...
                {visibleDepartures.map((d,i)=>{
                    ...
                    const plannedRaw = currentStop?.departure_time_planned
    || currentStop?.planned_departure_time
    || currentStop?.planned_time
    || d.planned_time
    || d.scheduled_departure_time
    || d.scheduled_time
    || null;

                    const expectedRaw = currentStop?.departure_time
    || currentStop?.departure_time_real
    || currentStop?.departure_time_estimated
    || d.departure_time
    || d.expected_departure_time
    || d.expected_time
    || d.arrival_time // fallback si aucune info de départ
    || null;
    ...
    ? stops.slice(currentStopIdx + 1)
    : [];
const names = stopsAfterGare.map(s => (typeof s === 'string') ? s : (s && (s.station_name || s.station)) || '');
                    ...
                    if (stops && stops.length) {
                        ...
                        if (typeof last === 'string') lastStopName = last;
                        else if (last && (last.station_name || last.station)) lastStopName = last.station_name || last.station;
                    }
                    const destination = lastStopName || d.terminus_name || d.destination || d.arrival_station || d.destination_station || 'Destination inconnue';
                    ...
                    <span className="time-pill-icon">
                        {d.cancelled ? <MdCancel/> : <FaClock/>}
                            ...
                            {/* Afficher la colonne voie uniquement si un quai est défini (non vide) */}
                            {platformToShow ? (
                                <div className="voiecol">
                                    <div className="voie-box-outer">
                                        <>
                                            <div className="voie-label">Voie</div>
                                            <div className="voie-letter">{platformToShow}</div>
                                        </>
                                    </div>
                                </div>
                            ) : null}
